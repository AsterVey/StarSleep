...
export function copyPlan(p:Plan):PlanInput {
  return {name:(p.name.slice(0,43)+' · 副本').slice(0,48),kind:p.kind,repeat:p.repeat,date:p.date,time:p.time,weekdays:[...p.weekdays],enabled:false};
}
export function selectPlans(plans:Snapshot['plans'],query:string,kind:string,status:string,sort:string){
  const search=query.trim().toLocaleLowerCase();
  return plans.filter(p=>(!search||p.name.toLocaleLowerCase().includes(search))&&(kind==='all'||p.kind===kind)&&(status==='all'||(status==='enabled'&&p.enabled&&!isFinished(p))||(status==='disabled'&&!p.enabled)||(status==='finished'&&isFinished(p))))
