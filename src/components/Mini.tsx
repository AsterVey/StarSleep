...
export function Mini({state,experience,busy,onFull,onPause,onPin,onHide}:{state:Snapshot;experience:ExperienceSnapshot;busy:boolean;onFull:()=>void;onPause:()=>void;onPin:()=>void;onHide:()=>void}){
 ...
 return <section className={`mini-console ${fault?'fault':''}`} aria-label="迷你控制台">
  <header className="mini-title"><Moon size={16}/><strong>星枢</strong>{state.safeMode&&<span className="mini-safe">安全测试 · 不关机</span>}<div><button aria-label={experience.prefs.miniPinned?'取消置顶':'置顶小窗'} disabled={!!experience.error} onClick={onPin}>{experience.prefs.miniPinned?<Pin size={15}/>:<PinOff size={15}/>}</button><button aria-label="隐藏到托盘" onClick={onHide}><X size={16}/></button></div></header>
  ...
  <div className="mini-bottom"><span>{state.paused&&state.pauseUntil?`恢复于 ${fullTime(state.pauseUntil)}`:next?`${state.paused?'原定 ':''}${fullTime(next.nextAt!)}`:'关闭此窗后仍在托盘运行'}</span><button aria-label={state.paused?'恢复计划':'暂停全部'} disabled={busy||fault} onClick={onPause}>{state.paused?<Play size={15}/>:<Pause size={15}/>}</button></div>
 </section>;
}
