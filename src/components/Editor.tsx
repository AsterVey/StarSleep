...
import type {Plan,PlanInput,QuickInput,Settings} from '../shared';
import {errorsFor,fullTime,nextTime} from '../plan-utils';
import {useClock} from '../clock';
import {Modal,Switch} from './UI';

export function Editor({initial,plan,copy=false,settings,onClose,onSave}:{initial:PlanInput;plan?:Plan;copy?:boolean;settings:Settings;onClose:()=>void;onSave:(p:PlanInput)=>Promise<boolean>}){
 ...
 return <><Modal title={copy?'复制计划':plan?'编辑计划':'新建时间计划'} onClose={close}><form noValidate onSubmit={async e=>{e.preventDefault();if(saving)return;setSubmitted(true);if(Object.keys(errors).length){document.querySelector<HTMLElement>('[aria-invalid="true"]')?.focus();return;}setSaving(true);try{await onSave(value);}finally{setSaving(false);}}}>
  ...
  <div className="modal-actions"><button type="button" className="secondary" onClick={close} disabled={saving}>取消</button><button className="primary" disabled={saving}><Check size={17}/>{saving?'正在保存…':'保存计划'}</button></div>
...
export function QuickPanel({initial,paused,settings,onClose,onSave}:{initial:QuickInput;paused:boolean;settings:Settings;onClose:()=>void;onSave:(input:QuickInput)=>Promise<boolean>}){
 ...
 return <Modal title="快捷计时" onClose={()=>{if(!saving)onClose();}}><form onSubmit={async e=>{e.preventDefault();if(!valid||saving)return;setSaving(true);try{await onSave(value);}finally{setSaving(false);}}}>
  <p className="muted">确认时开始计时，重新打开软件不会重置截止时间。</p><div className="quick-presets">{[[30,'30 分钟'],[60,'1 小时'],[120,'2 小时'],[240,'4 小时']].map(([n,label])=><button type="button" key={n} aria-pressed={value.minutes===n} onClick={()=>setValue({...value,minutes:Number(n)})}>{label}</button>)}</div>
  ...
  <div className="modal-actions"><button type="button" className="secondary" disabled={saving} onClick={onClose}>取消</button><button className="primary" disabled={!valid||saving}>{saving?'正在保存…':paused?'保存快捷计划':'开始计时'}</button></div>
 </form></Modal>;
}
