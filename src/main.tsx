...
import { Power, Bell, Plus, Settings2, Minus, X, Clock3, CalendarDays, Pencil, Trash2, Volume2, VolumeX, ChevronRight, ArrowUpRight, Check, RotateCcw, History, Info, Sparkles, Moon, CheckCircle2, AlertTriangle, Github } from 'lucide-react';
import '@fontsource/rajdhani/400.css';
import '@fontsource/rajdhani/500.css';
import '@fontsource/rajdhani/600.css';
import './style.css';
import './fit.css';
import type { Api, Kind, Plan, PlanInput, Settings, Snapshot } from './shared';

const empty:Snapshot={plans:[],warnings:[],alarms:[],logs:[],settings:{sound:true,volume:0.5,reducedMotion:false},now:Date.now(),safeMode:false};
...
function App(){
  ...
  return <div className="app" onClickCapture={e=>{if((e.target as HTMLElement).closest('button')&&state.settings.sound)tone('click');}}>
    ...
    <nav className="toolbar"><div className="page-name"><span className="active-dot"/><span>休息控制台</span><span className="divider"/><span className="muted">{state.safeMode?'安全测试模式':'本地运行'}</span></div><div className="toolbar-right"><time>{dateText(state.now)}<b>{timeText(state.now)}</b></time><button className="icon-button" aria-label="设置" title="设置" onClick={()=>setSettingsOpen(true)}><Settings2 size={19}/></button></div></nav>
    ...
    <main className="workspace">
      <section className="orbit-panel" aria-label="下一次关机">
        ...
        <div className={`orbital ${next?'armed':''}`}>
          ...
          <div className="dial-content"><div className="dial-icon"><Power size={25} strokeWidth={1.4}/></div><div className="dial-label">{next?(days>=4?`距离下次关机 · ${days} 天`:'距离下次关机'):'等待下一次休息'}</div><div className="countdown" aria-label={next?`剩余 ${days>=4?days+' 天 ':''}${parts.join(':')}`:'尚未设置关机'}>{parts.map((p,i)=><React.Fragment key={i}>{i>0&&<span className="colon">:</span>}<span>{next?p:'--'}</span></React.Fragment>)}</div><div className="dial-units"><span>小时</span><span>分钟</span><span>秒</span></div><div className={`dial-state ${next?'live':''}`}><span/>{next?'计划已就绪':'尚未设置关机计划'}</div></div>
        </div>
    ...
    <footer><span><span className="active-dot"/>{state.storageError?'自动执行已暂停':'运行期间计划有效'}<span className="footer-divider">/</span>不唤醒电脑 · 不补执行过期任务</span><button className="text-button" onClick={()=>setLogsOpen(true)}><History size={14}/> 执行记录 <ChevronRight size={14}/></button></footer>
    ...
    {settingsOpen&&<SettingsPanel onLink={target=>void openLink(target)} settings={state.settings} onClose={()=>setSettingsOpen(false)} onUpdate={s=>void run(()=>api!.settings(s))} onDemo={()=>void run(()=>api!.demo('alarm'))}/>}
    ...
    {busy&&<Reminder state={state} onAction={(keys,action,mins)=>void run(()=>api!.act(keys,action,mins))}/>}
  </div>;
}
function Editor({plan,onClose,onSave}:{plan:Plan|null;onClose:()=>void;onSave:(input:PlanInput)=>Promise<void>}){
  ...
  return <Modal title={plan?'编辑计划':'新建时间计划'} onClose={onClose}><form onSubmit={async e=>{e.preventDefault();setSaving(true);try{await onSave(value);}finally{setSaving(false);}}}>
    ...
    <div className="enable-line"><span>启用这个计划</span><Switch value={value.enabled} label="启用这个计划" onChange={()=>update('enabled',!value.enabled)}/></div>
...
function SettingsPanel({settings,onClose,onUpdate,onDemo,onLink}:{onLink:(target:'repository'|'author')=>void;settings:Settings;onClose:()=>void;onUpdate:(s:Settings)=>void;onDemo:()=>void}){return <Modal title="设置与关于" onClose={onClose}><div className="setting"><div><strong><Volume2 size={18}/> 按钮与操作音效</strong><p>短促的电子反馈，不影响闹钟响铃。</p></div><Switch value={settings.sound} label="按钮音效" onChange={()=>onUpdate({...settings,sound:!settings.sound})}/></div><div className="setting vertical"><label htmlFor="volume">提醒音量 <span>{Math.round(settings.volume*100)}%</span></label><input id="volume" type="range" min="0" max="1" step="0.05" value={settings.volume} onChange={e=>onUpdate({...settings,volume:Number(e.target.value)})}/><button className="text-button" onClick={onDemo}><Bell size={15}/> 试听闹钟</button></div><div className="setting"><div><strong><Sparkles size={18}/> 减少动态效果</strong><p>关闭背景轨道和鼠标波纹。</p></div><Switch value={settings.reducedMotion} label="减少动态效果" onChange={()=>onUpdate({...settings,reducedMotion:!settings.reducedMotion})}/></div><div className="settings-note"><Moon size={18}/><p>窗口隐藏后自动暂停装饰动画。<br/>软件不随 Windows 启动，也不会唤醒电脑。</p></div><section className="about-product" aria-label="关于星眠"><div className="about-title"><span className="brand-mark"><Moon size={22}/></span><div><h3>星眠 <span>Star Sleep</span></h3><p>版本 {PRODUCT.version} · Windows 桌面版</p></div></div><p className="author-line">第一作者 <button className="text-button" onClick={()=>onLink('author')}>{PRODUCT.author} <ArrowUpRight size={13}/></button></p><p className="author-bio">借助 AI，把日常问题做成实用、耐看的产品。<br/>让夜间的工作有个终点，也给电脑一段休息。</p><div className="about-links"><button className="secondary" onClick={()=>onLink('repository')}><Github size={16}/> GitHub 项目主页 <ArrowUpRight size={15}/></button><span>允许工作使用 · 禁止售卖</span></div></section></Modal>;}
function Reminder({state,onAction}:{state:Snapshot;onAction:(keys:string[],action:'cancel'|'snooze',minutes?:number)=>void}){
