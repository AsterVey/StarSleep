...
![v1.2.0 控制台：更紧凑的倒计时与更宽的计划管理区](docs/images/v1.2.0/console.png)

<p align="center"><sub>真实桌面截图；计划为测试示例，运行于安全测试模式。</sub></p>

## 选择你的版本

**普通用户安装软件：双击 `StarSleep-Setup-1.2.0.exe`。** 本地文件位于 `release/v1.2.0/`，无需另装 Node.js。
`StarSleep-Source-1.2.0.zip` 是开发源码，**不是安装包的压缩版**。
...
```powershell
npm run qa       # v1.2 桌面交互、隐藏音频与缩放验证
npm run package  # 构建 Windows x64 安装程序
npm run qa:performance # 与本地 v1.1.0 打包程序对比
npm run qa:upgrade     # 本地两版打包程序的数据兼容验证
```

`npm run dev` 仅提供浏览器界面预览。测试用 `--safe-mode` 替换实际关机执行器，测试数据与真实计划隔离。请勿去掉安全参数执行自动化测试。

性能与升级脚本需要本机已有 `release/v1.1.0/win-unpacked/` 的历史打包程序；升级脚本同时需要本版 `release/v1.2.0/win-unpacked/`。源码 ZIP 不包含这些可执行程序。
